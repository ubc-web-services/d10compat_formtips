...
class FormtipsSettingForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($this->themeHandler->listInfo() as $theme_key => $theme) {
      $themes[$theme_key] = $theme->info['name'];
    }
    $form['formtips_themes'] = [
      '#type' => 'select',
      '#title' => $this->t('Themes'),
      '#description' => $this->t('Select the themes for which formtips will be applied (selecting none will apply to all).'),
      '#options' => $themes,
      '#multiple' => TRUE,
      '#default_value' => $formtips_settings->get('formtips_themes'),
    ];
